...
import { Box, Typography, Stack, TextField, MenuItem, Select } from "@pankod/refine-mui";
  ...
  return (
    <Box>
      <Box mt="20px" sx={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
        <Stack direction='column' width="100%">
          <Typography fontSize={25} fontWeight={600} color="#11142d">
            {!allProperties.length ? 'There are no available properties' : 'All Properties'}
          </Typography>
          <Box mb={2} mt={3} display='flex' width="84%" justifyContent='space-between' flexWrap='wrap'>
            <Box display="flex" gap={2} flexWrap="wrap" mb={{ xs: '20px', sm: 0 }}>
              <CustomButton title={`Sort Price ${currentPrice === 'asc' ? '↑' : '↓'}`}
                handleClick={() => toggleSort('price')} backgroundColor='#475be8' color="#fcfcfc" />
              <TextField
                variant="outlined"
                color="info"
              ...
              >
                ...
                {['Apartment', 'Villa', 'Farmhouse', 'Townhouse', 'Studio', 'Chalet'].map((type) => (
      ...
      {allProperties.length > 0 && (
        <Box display="flex" gap={2} mt={3} flexWrap="wrap" justifyItems='center' alignItems='center' >
          <CustomButton
            title="Previous"
